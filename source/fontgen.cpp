...
int CFontGen::UpdateIconImage(int oldId, int id, const char *file, int xoffset, int yoffset, int advance)
{
	...
	if (iconImages.end() != iconImages.find(oldId))
	{
		isFound = true;

		if (NULL != iconImages[oldId])
		{
			if (oldId != id)
			{
				DeleteIconImage(id);
				iconImages[id] = iconImages[oldId];
				iconImages.erase(oldId);

				iconImages[id]->id = id;
				iconImages[id]->xoffset = xoffset;
				iconImages[id]->yoffset = yoffset;
				iconImages[id]->advance = advance;

				if (iconImages[id]->fileName != file)
				{
					// Load the image file
					acImage::Image rawImg;
					int r = LoadImageFile(file, rawImg);
					...
					iconImages[id]->fileName = file;
					delete iconImages[id]->image;
					iconImages[id]->image = image;
				}
			}
		}
	}
...
const map<int, SIconImage *>&CFontGen::GetIconImageInfo()
{
	return iconImages;
}
...
HFONT CFontGen::CreateFont(int FontSize)
{
	...
	TCHAR buf[1024];
	ConvertAnsiToTChar(fontName, buf, 1024);
...
void CFontGen::InternalGeneratePages()
{
	...
	int count = 0;
	for (map<int, SIconImage *>::iterator itr = iconImages.begin(); iconImages.end() != itr; ++itr, ++count)
	{
		int ch = itr->second->id;
		chars[ch] = new CFontChar();
		chars[ch]->CreateFromImage(count, itr->second->image, itr->second->xoffset, itr->second->yoffset, itr->second->advance);
...
int CFontGen::SaveConfiguration(const char *szFile)
{
	...
	FILE *f = 0;
	errno_t e  = fopen_s(&f, filename.c_str(), "wb");
	if( e != 0 || f == 0 ) return -1;
