...
{
	string name;
	ConvertTCharToAnsi(szFile, name);

	int selectedFilter = GetSelectedFilter() - 1;
	if (selectedFilter < filter.size())
	{
		string suffix = filter[selectedFilter];
		unsigned index = suffix.rfind('*');
		if (string::npos != index)
		{
			suffix.erase(0, index + 1);
		}
...
int CFileDialog::AskForOpenFileName(CWindow *owner)
{
	...
	ConvertAnsiToTChar(fileFilter, fileFilterBuf, 512);

	TCHAR initialDirBuf[512] = {0};
	ConvertAnsiToTChar(initialDir, initialDirBuf, 512);
...
{
	ConvertAnsiToTChar(name, szFile, 260);
}
