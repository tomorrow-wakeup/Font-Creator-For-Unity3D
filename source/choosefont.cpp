...
void CChooseFont::OnFontChange()
{
	...
	string charsetName;
	ConvertTCharToAnsi(buf, charsetName);
	...
	ConvertAnsiToTChar(str, buf, 256);
...
void CChooseFont::GetOptions()
{
	...
	ConvertTCharToAnsi(buf, font);

	GetDlgItemText(hWnd, IDC_FONTFILE, buf, 256);
	ConvertTCharToAnsi(buf, fontFile);
