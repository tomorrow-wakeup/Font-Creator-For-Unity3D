...
void CImageMgr::RefreshList()
{
	if( listView )
	{
		...
		const map<int, SIconImage *> &IconImage = fontGen->GetIconImageInfo();
		int count = 0;
		for (map<int, SIconImage *>::const_iterator itr = IconImage.begin(); IconImage.end() != itr; ++itr, ++count)
		{
			stringstream id_stream;
			id_stream << itr->first;

			listView->InsertItem(count, id_stream.str(), itr->first);

			TCHAR buf[512];
			ConvertAnsiToTChar(itr->second->fileName, buf, 512);
			ListView_SetItemText(listView->GetHandle(), count, 1, buf);
		}
	}
...
void CImageMgr::OnEditImage()
{
	...
	if( (item = listView->GetNextItem(item, LVNI_FOCUSED)) != -1 )
	{
		...
		map<int, SIconImage *>::const_iterator itr = IconImage.find(oldId);
		if (IconImage.end() != itr)
		{
			CIconImageDlg iconDlg;
			iconDlg.fileName = itr->second->fileName;
			iconDlg.id = itr->second->id;
			iconDlg.xoffset = itr->second->xoffset;
			iconDlg.yoffset = itr->second->yoffset;
			iconDlg.advance = itr->second->advance;
