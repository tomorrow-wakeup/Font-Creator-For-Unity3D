...
LRESULT CCharWin::MsgProc(UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		case ID_OPTIONS_LOAD_FONT_CONFIGURATION:
			OnLoadFontConfiguration();
			return 0;

		case ID_OPTIONS_SAVE_FONT_CONFIGURATION:
			OnSaveFontConfiguration();
			return 0;
...
void CCharWin::LoadFontConfig(const string &configFile)
{
	...
	file_parser.init(configFile.c_str());
	...
	while (file_parser.has_more_rows())
	{
		...
		if (9 == row.size())
		{
			...
			{
				TCHAR characterT[4];
				ConvertAnsiToTChar(character.c_str(), characterT, 4);
				id = (int)characterT[0];
			}
			...
			if (r != 0)
			{
				MessageBox(hWnd, ErrorT, __TEXT("File error"), MB_OK);
				return;
			}
		}
	}
