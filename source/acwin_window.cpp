...
int CWindow::Create(const char *title, int width, int height, DWORD style, DWORD styleEx, CWindow *parent, const char *className)
{
	...
	ConvertAnsiToTChar(title, titleBuf, 256);
	ConvertAnsiToTChar(className, classNameBuf, 128);
							 ...
							 HICON newIcon, HICON newSmallIcon, HCURSOR newCursor)
{
	...
	TCHAR buf[256];
	ConvertAnsiToTChar(className, buf, 256);
	WNDCLASSEX wndClass;
...
void CWindow::UpdateWindowText(const char *text)
{
	...
	string oldText;
	ConvertTCharToAnsi(str, oldText);
	if( oldText != text )
	{
		TCHAR newText[1024];
		ConvertAnsiToTChar(text, newText, 1024);
...
void ConvertTCharToAnsi(const TCHAR *buf, std::string &ansi)
{
#ifdef _UNICODE
	char bufUTF8[1024];
	WideCharToMultiByte(CP_ACP, 0, buf, -1, bufUTF8, 1024, NULL, NULL);
	ansi = bufUTF8;
#else
	ansi = buf;
#endif
...
void ConvertAnsiToTChar(const std::string &ansi, TCHAR *buf, size_t bufSize)
{
#ifdef _UNICODE
	MultiByteToWideChar(CP_ACP, 0, ansi.c_str(), ansi.size() + 1, buf, bufSize);
#else
	strcpy_s(buf, bufSize, ansi.c_str());
#endif
}
